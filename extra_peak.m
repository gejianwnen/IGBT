...
transient = struct([]);
collectorEmitterVoltage_peak = zeros(length(measurement.transient),1);
...
num = 1:length(measurement.transient);num = num';
peak_df = table(num,collectorEmitterVoltage_peak);
writetable(peak_df, [file_folder 'collectorEmitterVoltage_peak.csv']);
